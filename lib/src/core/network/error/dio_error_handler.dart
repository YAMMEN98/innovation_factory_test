    ...
    case DioErrorType.badResponse:
      {
        ...
        if (error.response?.data['code'] != null && (error.response?.data['code'] ?? "0") != "0") {
          ...
          if (error.response?.statusCode == 200 && ("${(error.response?.data["statusCode"] ?? "0")}" != "0")) {
            ...
            errorDescription = error.response?.data is String ? "403 Forbidden" : error.response?.data["message"] ?? "Unknown Error";
          } else if (error.response?.statusCode == 404) {
            errorDescription = error.response?.data is String ? "404 Unknown Error" : error.response?.data["message"] ?? "Unknown Error";
          } else if (error.response?.statusCode == 409) {
            errorDescription =
                error.response?.data["message"] + ",\n Minutes left to join: " + error.response?.data["data"]["mins_to_join"].toString() ??
                    "Unknown Error";
